  ...
  initWidgets() {
    if (!window[this.widgets.defaults.objectName].init) {
      ...
      if (
        ...
        scriptElement.dataset.cpCurrencyWidget
      ) {
      ...
      setTimeout(() => {
        ...
        return cpBootstrap.loop(mainElements, (element, index) => {
          ...
          promise = promise.then(() => {
            ...
            return newSettings.modules.indexOf("chart") > -1 &&
              !window.Highcharts
...
class widgetsClass {
  ...
  init(index) {
    if (!this.getMainElement(index)) {
      return console.error(
        'Bind failed, no element with class = "' +
  ...
  getDefaults(index) {
    return new Promise((resolve) => {
      ...
      if (mainElement && mainElement.dataset) {
        if (
          !mainElement.dataset.modules &&
          mainElement.dataset.version === "extended"
        )
          this.updateData(index, "modules", ["market_details"]);
        if (
          !mainElement.dataset.modules &&
          mainElement.dataset.version === "standard"
        )
  ...
  addWidgetElement(index) {
    ...
    promise = promise.then(() => {
      return cpBootstrap.loop(this.defaults.availableModules, (module) => {
        return this.states[index].modules.indexOf(module) > -1
    ...
    promise = promise.then(() => {
      return cpBootstrap.loop(modulesArray, (module) => {
        ...
        return label
          ? this[`widget${label}Element`](index).then(
    ...
    promise = promise.then(() => {
      return (mainElement.innerHTML =
        this.widgetMainElement(index) + modules + this.widgetFooter(index));
    });
    promise = promise.then(() => {
      ...
      return chartContainer
        ? chartContainer.parentElement.insertAdjacentHTML(
            "beforeend",
  ...
  getData(index) {
    const url =
      "https://api.coinpaprika.com/v1/widget/" +
      this.states[index].currency +
      "?quote=" +
      this.states[index].primary_currency;
    return fetchService
      .fetchData(url)
      .then((response) => {
  ...
  initInterval(index) {
    ...
    if (
      this.states[index].update_active &&
      this.states[index].update_timeout > 1000
    ) {
    ...
    if (mainElement) {
      ...
      if (key === "isData" || key === "message") {
        ...
        for (let k = 0; k < headerElements.length; k++) {
          headerElements[k].innerHTML = !state.isData
            ? this.widgetMainElementMessage(index)
            : this.widgetMainElementData(index);
        }
      } else {
          ...
          if (updateElement.classList.contains("cp-widget__rank")) {
            let className =
              parseFloat(value) > 0
                ? "cp-widget__rank-up"
                : parseFloat(value) < 0
                  ? "cp-widget__rank-down"
                  : "cp-widget__rank-neutral";
            ...
            if (value === undefined) {
              ...
              value =
                key === "price_change_24h"
                  ? "(" + cpBootstrap.round(value, 2) + "%)"
                  : cpBootstrap.round(value, 2) + "%";
            }
          }
          if (
            updateElement.classList.contains("showDetailsCurrency") &&
            !state.show_details_currency
          ) {
            value = " ";
          }
  ...
  updateWidgetTranslations(lang, data) {
    ...
    for (let x = 0; x < this.states.length; x++) {
      let isNoTranslationLabelsUpdate =
        this.states[x].noTranslationLabels.length > 0 && lang === "en";
        ...
        for (let y = 0; y < transalteElements.length; y++) {
          transalteElements[y].classList.forEach((className) => {
            if (className.search("translation_") > -1) {
              ...
              let labelIndex =
                this.states[x].noTranslationLabels.indexOf(translateKey);
  ...
  stylesheet() {
    if (this.defaults.style_src !== false) {
      let url =
        this.defaults.style_src ||
        this.defaults.origin_src + "/dist/" + this.defaults.cssFileName;
  ...
  widgetMainElementMessage(index) {
    ...
    return (
      '<div class="cp-widget__main-no-data cp-translation translation_message">' +
      this.getTranslation(index, message) +
      "</div>"
    );
  }
  ...
  widgetMarketDetailsElement(index) {
    return Promise.resolve(
      this.states[index].modules.indexOf("market_details") > -1
        ? '<div class="cp-widget__details">' +
  ...
  widgetAthElement(index) {
    return (
      "<div>" +
      '<small class="cp-translation translation_ath">' +
      this.getTranslation(index, "ath") +
      "</small>" +
      "<div>" +
      '<span class="price_athTicker parseNumber">' +
      cpBootstrap.emptyData +
      " </span>" +
      '<span class="symbolTicker showDetailsCurrency"></span>' +
      "</div>" +
      '<span class="percent_from_price_athTicker cp-widget__rank">' +
      cpBootstrap.emptyData +
      "</span>" +
      "</div>"
    );
  ...
  widgetVolume24hElement(index) {
    return (
      "<div>" +
      '<small class="cp-translation translation_volume_24h">' +
      this.getTranslation(index, "volume_24h") +
      "</small>" +
      "<div>" +
      '<span class="volume_24hTicker parseNumber">' +
      cpBootstrap.emptyData +
      " </span>" +
      '<span class="symbolTicker showDetailsCurrency"></span>' +
      "</div>" +
      '<span class="volume_24h_change_24hTicker cp-widget__rank">' +
      cpBootstrap.emptyData +
      "</span>" +
      "</div>"
    );
  ...
  widgetMarketCapElement(index) {
    return (
      "<div>" +
      '<small class="cp-translation translation_market_cap">' +
      this.getTranslation(index, "market_cap") +
      "</small>" +
      "<div>" +
      '<span class="market_capTicker parseNumber">' +
      cpBootstrap.emptyData +
      " </span>" +
      '<span class="symbolTicker showDetailsCurrency"></span>' +
      "</div>" +
      '<span class="market_cap_change_24hTicker cp-widget__rank">' +
      cpBootstrap.emptyData +
      "</span>" +
      "</div>"
    );
  ...
  widgetSelectElement(index, label) {
    ...
    for (let i = 0; i < this.states[index][label + "_list"].length; i++) {
      ...
      buttons +=
        '<button class="' +
        (data.toLowerCase() === this.states[index][label].toLowerCase()
          ? "cp-widget-active "
          : "") +
        (label === "primary_currency"
          ? ""
          : "cp-translation translation_" + data.toLowerCase()) +
        '" data-option="' +
        data +
        '">' +
        this.getTranslation(index, data.toLowerCase()) +
        "</button>";
    }
    if (label === "range");
    ...
    return (
      '<div data-type="' +
      label +
      '" class="cp-widget-select">' +
      '<label class="cp-translation translation_' +
      label +
      '">' +
      title +
      "</label>" +
      ...
      "cp-widget__capitalize cp-translation translation_" +
      this.states[index][label].toLowerCase() +
      '">' +
      this.getTranslation(index, this.states[index][label].toLowerCase()) +
      "</span>" +
  ...
  widgetFooter(index) {
    ...
    return !this.states[index].isWordpress
      ? '<p class="cp-widget__footer cp-widget__footer--' +
          index +
          '">' +
          '<span class="cp-translation translation_powered_by">' +
          this.getTranslation(index, "powered_by") +
          " </span>" +
          '<img style="width: 16px" src="' +
          this.main_logo_link() +
          '" alt=""/>' +
          '<a target="_blank" href="' +
          this.coin_link(currency) +
          '">coinpaprika.com</a>' +
          "</p>"
      : "";
  }
  ...
  getImage(index) {
    let data = this.states[index];
    let imgContainers =
      data.mainElement.getElementsByClassName("cp-widget__img");
  ...
  main_logo_link() {
    return (
      this.defaults.img_src ||
      this.defaults.origin_src + "/dist/img/logo_widget.svg"
    );
  }
  ...
  getTranslation(index, label) {
    let text = this.defaults.translations[this.states[index].language]
      ? this.defaults.translations[this.states[index].language][label]
      : null;
  ...
  getTranslations(lang) {
    if (!this.defaults.translations[lang]) {
      const url =
        this.defaults.lang_src ||
        this.defaults.origin_src + "/dist/lang/" + lang + ".json";
...
class chartClass {
  constructor(container, state) {
    ...
    this.defaultOptions = {
      chart: {
        ...
        events: {
          render: (e) => {
            if (this?.chart_data?.price && this?.chart_data?.price?.length > 0) {
            ...
            if (e.target.annotations) {
              ...
              cpBootstrap.loop(chart.annotations.allItems, (annotation) => {
                let y =
                  ...
                  chart.spacing[0] -
                  2 -
                  (this.isResponsiveModeActive(chart) ? 10 : 0);
          ...
          events: {
            legendItemClick: (event) => {
              if (event.browserEvent.isTrusted) {
                if (
                  this.chartsWithActiveSeriesCookies.indexOf(
                    event.target.chart.renderTo.id,
                  ) > -1
                )
                  this.setVisibleChartCookies(event);
              }
  ...
  bind(chart) {
    ...
    promise = promise.then(() => {
      return this.customDate
        ? this.fetchDataPackage(this.startDate, this.endDate, true)
    ...
    promise = promise.then(() => {
      return this.callback
        ? this.callback(this.chart, this.defaultRange)
        : null;
    });
    return promise;
  }
  ...
  fetchDataPackage(minDate, maxDate, initial = false) {
    ...
    promise = promise.then(() => {
      if (this.options.cpEvents) {
        let url = isPreciseRange
          ? this.getNavigatorExtremesUrl(minDate, maxDate, "events")
          : this.getExtremesDataUrl(this.id, "events") +
            "/" +
            this.getRange() +
            "/";
    ...
    promise = promise.then(() => {
      let url =
        (isPreciseRange
          ? this.getNavigatorExtremesUrl(minDate, maxDate)
          : this.asyncUrl.replace("_range_", this.getRange())) +
        this.asyncParams;
      ...
      .then((response) => {
        ...
        return response.json().then((data) => {
          ...
          promise = promise.then((content) => {
            return replace
              ? this.replaceData(content.data, dataType)
              : this.updateData(content.data, dataType);
          });
          return promise;
        });
      })
  ...
  hideChart(bool = true) {
    ...
    promise = promise.then((result) => {
      return cpBootstrap.loop(result, (element) =>
        element.classList[classFunc]("cp-hidden"),
      );
    });
    promise = promise.then(() => {
  ...
  toggleEvents() {
    ...
    if (this.options.cpEvents) {
      ...
      promise = promise.then((elements) => {
        return cpBootstrap.loop(elements, (element) => {
          if (this.isEventsHidden) {
            return !element.classList.contains("highcharts-annotation__hidden")
              ? element.classList.add("highcharts-annotation__hidden")
              : null;
          }
          return element.classList.contains("highcharts-annotation__hidden")
            ? element.classList.remove("highcharts-annotation__hidden")
            : null;
        });
      });
      promise = promise.then(() => {
      ...
      promise = promise.then((elements) => {
        return cpBootstrap.loop(elements, (element) => {
          if (this.isEventsHidden) {
            return !element.classList.contains("highcharts-plot-line__hidden")
              ? element.classList.add("highcharts-plot-line__hidden")
              : null;
          }
          return element.classList.contains("highcharts-plot-line__hidden")
            ? element.classList.remove("highcharts-plot-line__hidden")
            : null;
        });
      });
    }
        ...
        case "data":
          ...
          return cpBootstrap.loop(Object.entries(data), (value) => {
            ...
            newData[value[0]] = oldData
              .filter((element) => {
                return (
                  value[1].findIndex((findElement) =>
                    this.isTheSameElement(element, findElement, dataType),
                  ) === -1
                );
              })
              .concat(value[1])
              .sort((data1, data2) =>
                this.sortCondition(data1, data2, dataType),
              );
          });
        case "events":
          newData = [];
  ...
  replaceData(data, dataType) {
    ...
    promise = promise.then(() => {
      return this.replaceCallback
        ? this.replaceCallback(this.chart, data, this.isLoaded, dataType)
        : null;
    });
    return promise;
  }
  ...
  replaceDataType(data, dataType) {
    switch (dataType) {
      case "data":
        ...
        return cpBootstrap.loop(Object.entries(data), (value) => {
          if (this.isExcluded(value[0])) return;
          return this.chart.get(value[0])
            ? this.chart.get(value[0]).setData(value[1], false, false, false)
  ...
  tooltipFormatter(pointer, label = "", search) {
    ...
    const header =
      '<div class="cp-chart-tooltip-currency"><small>' +
      new Date(pointer.x).toUTCString() +
      "</small><table>";
    ...
    pointer.points.forEach((point) => {
      ...
      content +=
        ...
        '<svg class="cp-chart-tooltip-currency__icon" width="5" height="5"><rect x="0" y="0" width="5" height="5" fill="' +
        point.series.color +
  ...
  getNavigatorExtremesUrl(minDate, maxDate, dataType) {
    let extremesDataUrl = dataType
      ? this.getExtremesDataUrl(this.id, dataType)
      : this.extremesDataUrl;
    return minDate && maxDate && extremesDataUrl
      ? extremesDataUrl + "/dates/" + minDate + "/" + maxDate + "/"
      : null;
  }
